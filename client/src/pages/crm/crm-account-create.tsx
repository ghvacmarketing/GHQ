...
export default function CrmAccountCreate() {
  ...
  const [potentialDuplicates, setPotentialDuplicates] = useState<Array<{
    id: string;
    name: string;
    phone: string | null;
    email: string | null;
  ...
  const checkValidation = (step: FormStep): { valid: boolean; errors: string[] } => {
    ...
    switch (step) {
      ...
      case 3:
        ...
        break;
      case 4:
        // No specific validation for step 4 anymore (lead/sales funnel removed)
        break;
      case 5:
  ...
  const checkForDuplicates = async (): Promise<boolean> => {
    ...
    try {
      const fullAddress = `${formData.address1}${formData.address2 ? ', ' + formData.address2 : ''}, ${formData.city}, ${formData.state} ${formData.zip}`;
      const phone = formData.accountType === "PROPERTY_MANAGER" ? formData.pmMainOfficePhone : formData.phone;
      const email = formData.accountType === "PROPERTY_MANAGER" ? formData.pmMainOfficeEmail : formData.email;
      
      const res = await apiRequest("POST", "/api/crm/customers/check-duplicates", {
        name: formData.displayName,
          ...
          <div className="flex gap-2">
            {currentStep < 5 ? (
              ...
              <Button
                ...
                disabled={!checkValidation(5).valid || createCustomerMutation.isPending || isCheckingDuplicates}
      ...
      <Dialog open={showDuplicateDialog} onOpenChange={setShowDuplicateDialog}>
        <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          <DialogHeader>
            ...
            <DialogDescription>
              We found {potentialDuplicates.length} existing customer(s) that may match. Please review them before creating a new customer.
            </DialogDescription>
          </DialogHeader>
                  ...
                  <TableRow key={dup.id}>
                    <TableCell>
                      <div>
                        ...
                        {dup.fullAddress && (
                          <p className="text-xs text-slate-500 truncate max-w-[200px]">{dup.fullAddress}</p>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="text-sm">
                        ...
                        {dup.email && <p className="text-slate-500">{dup.email}</p>}
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => window.open(`/crm/customers/${dup.id}`, '_blank')}
                      >
