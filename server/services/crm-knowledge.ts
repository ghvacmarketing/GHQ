...
- A quote's line items are split: CUSTOMER line items (what the customer sees) and INTERNAL COSTS (labor, warranty reserve, worksheet cost build-up) shown in a separate amber "Internal costs" card on the quote detail page
- For Custom Pricing quotes the Internal costs card is ONE integrated breakdown (no separate items table — the cost lines ARE the build-up, so nothing is listed twice): grouped cost build-up (Materials & equipment lines, Labor with a payroll/benefits split, warranty reserve — each line keeps its show-to-customer/delete actions) → direct cost → sell price → WHERE THE PRICE GOES with a color-segmented share bar (direct cost + overhead / financing / commission / profit) → gross profit + margin %, crew days + GP per crew day, install type, and discount math when given. EVERY figure shows its percentage of the sell price beside the dollars. Service-mode quotes show their equivalent build-up (parts, shrinkage, labor, sales tax, warranty reserve, GHVAC warranty coverage). Older quotes without a stored snapshot fall back to the plain items table with a sell − costs margin readout
- RE-EDITING A CUSTOM QUOTE: the Internal costs card has an "Edit in Custom Pricing" button that re-opens the finalized quote in the worksheet (/crm/quotes/install-worksheet/<quoteId>) with every input, line, and calculated total restored — works on draft AND sent quotes, any time until the customer accepts (accepted/converted quotes are locked). Saving regenerates the quote's line items, totals, and costing snapshot in place; the customer/salesperson never get re-asked. Very old service quotes created before the worksheet stored its raw state can't be re-opened (the button hides)
...
- The Discounts dialog (quote detail → line items → "Discounts" button) manages everything in one place: it LISTS the discounts already on the quote (each removable with an ✕ right in the dialog), and below that adds a new one — pick Promotion (custom $ or %) or Maintenance (fixed 15%). On an options quote the targets are tappable option TILES (all selected by default); each selected tile live-previews exactly what comes off that option's own price as you type. The dialog stays open after applying so discounts stack in one sitting (e.g. 20% off Best, then $500 flat off Good, then another 5% everywhere). Rules: PROMOTION discounts stack — any number per option; maintenance and a protection bundle's parts discount are one per option Each option card shows its struck-through original price, its own discount lines, and "You save $X"; the quote detail's line-items section lists every discount by name per option (subtotal → each discount → option total), and an untagged discount line is badged "All options"
...
- update_lead — edits the customer's lead: move it through stages (new → contacted → quote sent → negotiating → won/lost), change temperature, value, salesperson, or notes; marking it lost records the reason. Changes show before/after on the approval card, and nothing is saved until approved.
...
- **Gibbs** (/crm/settings/gibbs, owner/admin only) — oversight for the assistant, three tabs: CONVERSATIONS (read any team member's Gibbs chats, read-only — same data as the assistant's review mode; approvals always stay with the chat's owner), ACTIONS (every action Gibbs proposed or ran across the whole team — who, what, when, outcome Ran/Awaiting approval/Dismissed/Replaced, with links to what was created; filter by user), and WHAT GIBBS CAN DO (the full capability list — every approval-gated action and live lookup — plus an UPDATES timeline of shipped and planned Gibbs features)
...
- **Package Pricing** (/crm/settings/packages) — TWO tabs, with a PRICING GIBBS side panel that slides in over the right edge of the page WITHOUT moving the content (toggled by the Ask Gibbs button in the header; small screens open the full assistant instead): the SAME Gibbs brain and the SAME approval rules, scoped to pricing work, with live screen context so "this package" refers to whatever is selected. Panel conversations file into the shared "Pricing" space and appear in the main nav Gibbs's sidebar — the nav Gibbs remains the primary assistant and sees everything. "Costs & Catalog": PACKAGE EQUIPMENT (master-detail: one Filters popover holds system type (SHP/GP/PHP/etc.), tier (Essential/Premium/Ultimate/...), tonnage, and an Unmatched-only toggle (active count on the button, Clear filters inside), plus free search by package/model; pick any active proposal-builder package → its exact models with images costed live from the catalog (every card is EDITABLE — hover pencil, or a dashed Add tile for empty slots: change the model number, the description proposals show, and the image via upload; prices never move from a card edit); an ADD PACKAGE button in the toolbar and a DUPLICATE… button in the breakdown rail create whole new packages via the same New Package dialog the builder's edit mode uses; FINANCING MATH sits under the package card — the stored "as low as" monthly, its effective factor, and the app-wide GreenSky-program estimate (12.99% APR · 120 mo); the right rail is the COST BREAKDOWN estimate waterfall: price − equipment − labor − materials − commission − financing buydown − overhead = estimated profit with margin % vs target, segmented share bar, a "suggested price at target margin" guidance line, and COST DRIFT baked in — when live equipment cost moved since the package was last priced it shows the drift with "Keep price" (re-baseline) and "Set price…" (new total + monthly with a GreenSky-program suggestion); that Set price flow is THE way package prices are changed; a FIX MATCHES workbench (amber button, or click any part's "not in catalog") lists every unmatched model string with usage counts and scored catalog suggestions — per row: map to a catalog model, search the catalog, add it to the catalog with a cost (auto-matches), clear junk placeholders, or skip; one Apply runs the batch; Gibbs can do this conversationally via the unmatched_package_models lookup + the remap_package_models BUILDER action, approval-gated like all actions). Below it the JOB COST MODEL (crew hours + per-system-type overrides, loaded $/hr rate, materials % of equipment, commission %, buydown %, overhead %, target margin % — edits preview the waterfall LIVE, Save persists; estimates only, prices never auto-change; Gibbs answers economics via the package_economics tool incl. whatIf scenarios; plus COSTING OVERRIDE GROUPS — named groups that give chosen system types and/or specific packages their own numbers, e.g. GP installs at a different labor rate than SHP: blank fields inherit the shop defaults, a package listed directly beats a system-type match, and any covered package's breakdown shows an amber "Custom costing — {group}" panel listing EXACTLY which values changed vs the shop defaults, with amber dots on the affected waterfall lines), then the EQUIPMENT CATALOG (every supplier model + cost, searchable, DOWNLOADABLE any time as a spreadsheet via the Download button; costs are READ-ONLY by hand — they only change via price-file uploads; retire/restore and add-model are available; new brands are just rows — no development per brand). "Proposal Builder" (middle tab — THE place to add packages from Settings): PACKAGES BY SECTION shows every package as a foldable tree of the builder's four layers — collapsible SECTION headers, collapsible TIER groups inside, then size · level rows — with a "Create package" button (blank, starts at layer 1), "Add to {section}" and "Add to {tier}" buttons scoped to where you clicked, Expand/Collapse all, and Duplicate on every row; below it the SECTION MANAGER reorders/renames/re-blurbs/hides the builder's system-type sections (new types appear automatically once their first package exists). Save buttons across pricing settings stay DISABLED until something actually differs from the saved state — reverting your edits re-disables them. "Price File Update": upload the supplier flat file (CSV/Excel; title/blank rows handled automatically) → reviewed four-bucket diff — price changes, new models, missing/discontinued models, and MODEL SUCCESSIONS (confirming swaps the model number inside every package; names, images, and prices stay untouched) — with journaled import history that archives the EXACT uploaded file for re-download (download icon on each history row). REMOVED 2026-08 (do NOT claim these exist): the CSV package import, bulk % price adjustments, and the editable price table — repricing is per-package via Set price in Package Equipment.
...
- **Sensors** (/crm/settings/sensors) — the control room for environment monitoring: every Govee sensor in one table (live reading, online dot, threshold summary, a Monitoring on/off switch that pauses polling, and a Settings button for per-sensor name/location/mapping/thresholds/calibration), plus the ALERT NOTIFICATION POLICY — which roles get notified (checkboxes, default Owner + Admin), how long a sensor must be continuously offline before an offline alert opens (default 30 min) and back online before it clears (default 10 min), and re-notify cooldowns per sensor+alert type (default 6h offline / 1h temp-humidity, 0 = notify every alert). Also shows the last 30 alerts with severity/status and whether each SENT a notification or was MUTED by cooldown. Saving applies within a minute — no redeploy.
...
The Environment Monitoring page (/crm/analytics, sidebar: Operations) is a live humidity/temperature dashboard fed by Govee sensors installed at customer properties (e.g. crawlspaces). Overall health status (normal/watch/high/critical/offline), a "Sync now" button, open alerts that can spawn service work orders straight into the dispatch queue, and device mapping that ties each sensor to a customer property. Sensor detail shows live readings, risk badge, a reading-history trend chart with threshold lines, and a Create Work Order button. Sensor management lives in Settings > Sensors (/crm/settings/sensors) — see the Settings section; the same per-sensor settings dialog also opens from a sensor card here via the "Sensor settings" button (name, location type incl. walk-in cooler/freezer, customer/property mapping, alert thresholds — High/Critical RH%, Temp low/high °F, leave blank to turn that alert off, e.g. blank-or-32 low and 45 high for a walk-in cooler — and calibration offsets). Alert notifications go to the roles picked in Settings > Sensors (default OWNER + ADMIN only) and are spam-guarded: an offline alert only fires after the sensor has been continuously offline for the configured window (default 30 min — brief blips are ignored) and clears after it's back online (default 10 min); threshold alerts resolve with a small hysteresis margin; repeat notifications for the same sensor+alert type are muted for a configurable cooldown (default 6h offline / 1h temp-humidity) even if the condition flaps — alert rows still always appear on the dashboard.
...
## INSTALL PLANNER
The Install Planner (/crm/install-planner) is a capacity-planning board for tentative and sold installs. Calendar view: drag across days to create a hold, drag/resize blocks, cross month boundaries. Timeline view: Gantt-style bars per crew, draggable across dates and crews; crews can be added/renamed/deleted. CLICKING (or tapping) any event — calendar bar, timeline bar, or timeline row — opens its edit dialog: name, dates, customer, crew, estimated value, confidence, notes; sold events edit there too (drag is reserved for tentative ones), and editing a sold event's dates also moves the linked project's schedule. Moving is HOLD-then-drag: press and hold a block ~a third of a second (it highlights with a ring when grabbed), then drag to move or resize — a quick click, even a sloppy one, always edits and never moves the schedule. The dialog's Options menu holds Mark sold, Open project (sold events), and Remove. Blocks carry confidence (High/Medium/Low, dashed) or Sold (solid green) styling; a "Crews/day" setting caps daily capacity. A "Sell" action converts a hold to Sold and creates/links an INSTALL project.

## PROPOSAL BUILDER
The Proposal Builder (/crm/quotes/proposal, launched from Quotes) is a step-by-step wizard for building install proposals in front of the customer. Steps: pick the unit/system type → tonnage (skipped for Mini-Split, Ducting, and Crawlspace work) → equipment tier from the price book → review the package. It generates a professional proposal (AI-formatted: package description, what's-included by category, line items, warranties & terms, financing text, next steps) that can be presented, saved as a quote, and printed as a PDF. Templates for the written portions are managed in Settings → Proposal Templates (merge fields + image library). The Exit button returns to Quotes (or to the launching project). BUILDER EDIT MODE (owner/admin/supervisor): an "Edit" toggle in the builder header — OFF by default so the customer-facing flow stays clean — reveals in-place authoring on every step: "New system type" card on step 1 (creating its first package adds the section), "Package in a new tier" on the tier step, "New size" on the tonnage step, dashed "Add package here" tiles plus a per-card DUPLICATE button on the package step, and an "Add the first package" button on the empty state. All of these navigate to the full-page Create/Duplicate Package WIZARD (/crm/settings/packages/new) and return to the builder afterward — note the builder's step position resets on return (the cart is preserved). It is one Create/Duplicate Package WIZARD — a FULL PAGE at /crm/settings/packages/new (not a popup; the header back arrow cancels and a ?back param returns you where you started) — five screens mirroring the builder's own order (1 System, 2 Tier, 3 Size — tonnage/BTU/any denomination, 4 Level, 5 Price): one question per screen, existing values as tappable chips scoped to the layers above (tapping one auto-advances) plus an always-available type-a-new-one field (no dropdowns — every layer accepts brand-new values); the left rail shows all five steps with the chosen value under each and is clickable to jump back and change anything; "Add to {section}/{tier}" opens at the first unanswered layer, Duplicate opens on Price with everything filled; Price screen has total price with a price÷67 monthly estimate and the optional equipment models; duplicates copy models, prices, and images that creates a real pricebook package instantly — no redeploy, and cost drift/catalog matching/costing overrides apply to it like any other package. The system-type step's section names, blurbs, ORDER, and visibility are data too — managed in Settings → Package Pricing → the "Proposal Builder" TAB (Packages by Section with Add/Duplicate everywhere + the section manager; new types appear there automatically once their first package exists; hidden sections disappear from the builder).
