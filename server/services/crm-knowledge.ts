...
The Environment Monitoring page (/crm/analytics, sidebar: Operations) is a live humidity/temperature dashboard fed by Govee sensors installed at customer properties (e.g. crawlspaces). Overall health status (normal/watch/high/critical/offline), a "Sync now" button, open alerts that can spawn service work orders straight into the dispatch queue, and device mapping that ties each sensor to a customer property. Sensor detail shows live readings, risk badge, a reading-history trend chart with threshold lines, and a Create Work Order button. Per-sensor settings live HERE, not in the Settings app: open a sensor card and click "Sensor settings" to edit its name, location type, customer/property mapping, alert thresholds (High/Critical RH%, Temp low/high °F — leave a threshold blank to turn that alert off, e.g. blank or ~32 low and ~45 high for a walk-in cooler), and calibration offsets. Alert notifications push to OWNER and ADMIN roles only (techs/sales never get them), and they're spam-guarded: an offline alert only fires after the sensor has been offline 30+ minutes straight (brief blips are ignored) and clears after 10 minutes back online; threshold alerts resolve with a small hysteresis margin; and repeat notifications for the same sensor+alert type are muted for a cooldown window (6h offline, 1h thresholds) even if the condition flaps — the alert rows still appear on the dashboard.
