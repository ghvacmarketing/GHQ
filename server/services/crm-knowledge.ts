...
- **Package Pricing** (/crm/settings/packages) — three tabs. "Packages & Pricing": bulk % adjustments, the price table, and COST DRIFT (live component cost vs the baseline snapshot behind each package's price — prices never change themselves; owners "Keep price" to accept new costs or set a new price, and an eye icon shows a live preview of the package exactly as the proposal builder presents it). "Packages & Pricing" also holds the JOB COST MODEL: a handful of shop-level numbers (install crew hours + per-system-type overrides, loaded $/hr labor rate, materials % of equipment, commission % of price, financing buydown/dealer-fee % of price, overhead % of price, and a target profit margin %) that turn each package's price into the full estimated waterfall shown in Package Equipment — ESTIMATES ONLY, never changes a price. Gibbs can answer package-economics questions live with the package_economics tool (filter by unitType/tier/tonnage/packageLevel, onlyBelowTarget, and whatIf scenario overrides like a higher labor rate). "Equipment Catalog": PACKAGE EQUIPMENT map (master-detail: pick any active proposal-builder package to see the exact models inside it — outdoor/coil/indoor-heat/thermostat with images — each costed live from the catalog, a COST BREAKDOWN — estimate waterfall (price − equipment − labor − materials − commission − buydown − overhead = estimated profit + margin % vs target, with a segmented share bar and a "suggested price at target margin" line that is guidance only), and FINANCING MATH: the package's stored "as low as" monthly with its effective factor vs the app-wide GreenSky-program estimate; searchable + filterable by system type) above the catalog itself: every supplier model + cost (searchable, inline cost edits, retire/restore; new brands are just rows — no development per brand). FINANCING CONVENTION app-wide: quotes, proposals, and the public quote page estimate monthly financing from the primary GreenSky program — 12.99% APR for 120 months, standard amortization (≈1.49% of the total per month; defined once in client/src/lib/financing.ts) — until GreenSky approval sets real terms; package cards show their own stored monthly. "Price File Update": upload the supplier flat file (CSV/Excel, brand·model·cost, columns mapped on screen) and review a four-bucket diff — price changes, new models, missing/discontinued models, and suggested MODEL SUCCESSIONS (old→new model number with a confidence score; confirming swaps the model inside every package that used it while names, images, and prices stay untouched). Every import is journaled with history.
