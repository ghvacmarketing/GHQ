...
-   **Protection & Care Plans**: Installation Protection Bundles are fixed-price quote add-ons in a dedicated "Protection Plans" Items category (`crmItemCategoryEnum` value `protection`); four bundles (Basic/Standard/Advanced/Elite) are seeded with coverage details in the description. Three monthly "Care" plans (Essential/Priority/Elite) are seeded as `customAgreementTypes` templates (frequency `monthly`; `visitsPerPeriod` holds the per-year visit count with the real annual cadence noted in the description). Idempotent check-by-name seeds run at startup in `server/index.ts` (`runProtectionAndCarePlanSeeds`).
    -   **Auto parts discount**: When a Protection bundle is added to a quote (create or detail page), the tech is prompted to apply the bundle's parts-discount % (Basic 5% / Standard 10% / Advanced 15% / Elite 20%) to eligible items (all priced, non-discount line items except the bundle itself). Applying adds a discount line with `discountKind: "protection"`; the discount amount auto-recalculates when eligible items change and is removed automatically when the bundle is removed. The % is parsed from the bundle description ("X% parts discount") with a name-keyword fallback; protection bundle line items use `lineType: "protection"`. Shared logic: `client/src/lib/protection-discount.ts`.
