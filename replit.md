...
-   **Maintenance Agreements System**: Flexible billing frequencies (weekly, monthly, annual), configurable visits, regional reminders, auto-creation from paid invoices, and **automatic invoice generation**. A daily scheduled job checks for agreements with `nextInvoiceDate` = today, creates invoices with the agreement price, emails them to customers via Resend, and updates `nextInvoiceDate` to the next billing cycle. Admins can manually trigger renewal processing via the "Process Renewals" button on the Agreements page.
